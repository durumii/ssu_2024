...
def get_lottery_nums():
    #임의적으로 로또 번호를 생성하는 함수
    lottery = []
...
def get_bonus_num(lottery):
    #보너스 번호를 생성하는 함수
    while True:
        num_bonus = random.randint(1, 45)
        if num_bonus not in lottery:
            #리스트 lottery에 있는 숫자와 보너스 번호가 겹치면 안됨
            return num_bonus

def get_user_nums():
    #직접 입력하겠다고 선택한 경우 사용자에게 번호를 입력받는 함수
    num_user = []
...
def auto_generate_user_nums():
    #직접 입력하지 않겠다고 선택한 경우 임의적으로 번호를 생성하는 함수
    num_user = []
    
    while len(num_user) < 6:
        n = random.randint(1, 45)
        if n not in num_user:
            time.sleep(1) #time 함수를 이용해 1초에 한번씩 숫자를 추가하게 함
            print("%d를 추가합니다." % n)
            num_user.append(n)
        else:
            print("%d는 중복입니다." % n)
...
def check_results(lottery, num_bonus, num_user):
    ...
    if common_num <= 2:
        ...
        return "5등 당첨입니다. 당첨금은 5,000원입니다."
    elif common_num == 4:
        return "4등 당첨입니다. 당첨금은 50,000원입니다."
    elif common_num == 5:
        if num_bonus in num_user:
            return "2등 당첨입니다. 당첨금은 47,338,261원입니다."
        else:
            return "3등 당첨입니다. 당첨금은 1,598,393원입니다."
    else:
        return "1등 당첨입니다. 당첨금은 2,556,266,046원입니다."
    
while True: #while True를 사용하면 break가 나올 때까지 무한반복이 이루어짐
